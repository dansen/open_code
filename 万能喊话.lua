...
function syninputinit()
    ...
    while true do
        ...
        mSleep(1000)
        userchoice =
            dialogRet("您已完成输入框初始化，是否需要再次确定", "", "需要", "不需要", 0)
...
function synenterinit()
    dialog(
        "请在确定后点击回发出，因为某些ios系统并不允许调用默认发出，以便系统锁定发出位置",
        0)
    while 1 do
        local x, y = catchTouchPoint()
        userchoice = dialogRet("您已完成发送键输入框初始化，是否需要再次确定", "", "需要",
            "不需要", 0)
        if userchoice == 1 then
        else
            return x, y
...
function worksort(mode)
    ...
    for i = 1, len do
        ...
        if type(sort[i]) == "number" then
        else
            lua_exit()
            dialog("错误的模式输入，在模式里面的第" .. i .. "个模式", 0)
        end
        if sort[i] > 5 then
            lua_exit()
            dialog("错误的模式输入，在模式里面的第" .. i .. "个模式", 0)
